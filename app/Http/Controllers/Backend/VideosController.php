...
class VideosController
{
    ...
    {
        $videos = Song::withoutGlobalScopes();
        ...
        $total_videos = 0;//$songs->count();

        if ($this->request->has('results_per_page'))
        {
            $videos = $videos->paginate(intval($this->request->input('results_per_page')));
        } else {
        ...
        return view('backend.videos.index')
            ->with('songs', $videos)
            ->with('total_videos', $total_videos);
    }
	...
	public static function getVoiceSkinsList($language){
		$curl = curl_init();
		curl_setopt_array($curl, array(
		  CURLOPT_URL => "https://api.lovo.ai/v1/skins?language=".$language,
