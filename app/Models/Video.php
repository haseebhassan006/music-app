...
class Video extends Model implements HasMedia
{
    ...
    protected $fillable = [
        'title', 'mood', 'album_id', 'artworkId', 'releasedOn', 'copyright', 'approve'
    ];
    ...
    public function getArtworkUrlAttribute($value)
    {
        ...
        if(! $media) {
            if(config('settings.automate')) {
                $row = \App\Models\VideoLog::where('video_id', $this->attributes['id'])->first();
                if(isset($row->id)) {
    ...
    {
        return route('frontend.video', ['id' => $this->attributes['id'], 'slug' => str_slug($this->attributes['title']) ? str_slug($this->attributes['title']) : str_replace(' ', '-', $this->attributes['title'])]);
    }
                ...
                switch ($permission) {
                    case 1:
                        if(isset($this->attributes['hls']) && $this->attributes['hls']) {
                            return route('frontend.video.stream.hls', ['id' => $this->attributes['id']]);
                        } else {
                            return URL::temporarySignedRoute('frontend.video.stream.mp3', now()->addDay(), [
    ...
    {
        return Album::leftJoin('album_videos', 'album_videos.album_id', '=', 'albums.id')
            ->select('albums.*', 'album_videos.id AS host_id')
            ->where('album_videos.video_id', '=', $this->id)
            ->first();
    }
    ...
    {
        DB::table('playlist_videos')->where('video_id', $this->id)->delete();
        DB::table('album_videos')->where('video_id', $this->id)->delete();
