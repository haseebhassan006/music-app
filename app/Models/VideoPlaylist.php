...
use DB;
use Auth;
class Videoplaylist extends Model implements HasMedia
{
    use InteractsWithMedia, SanitizedRequest;

    protected $appends = ['artwork_url', 'permalink_url'];

    protected $hidden = ['media', 'user_id', 'mood', 'approved', 'updated_at'];
    ...
    public function getArtworkUrlAttribute($value)
    {
        ...
        if(! $media) {
            if(config('settings.automate')) {
                $row = \App\Models\VideoPlaylistLog::where('videoplaylist_id', $this->attributes['id'])->first();
                if(isset($row->id)) {
    ...
    {
        return route('frontend.playlist', ['id' => $this->attributes['id'],'slug' => str_slug($this->attributes['title'])]);
    }

    public function videos()
    {
        return Video::leftJoin('playlist_videos', 'playlist_videos.video_id', '=', (new Video)->getTable() . '.id')
            ->select('videos.*', 'playlist_videos.id as host_id')
            ->where('playlist_videos.videoplaylist_id', $this->id)
            ->orderBy('playlist_videos.priority', 'asc');
    }
    ...
    {
        DB::table('playlist_videos')->where('videoplaylist_id', $this->id)->delete();
