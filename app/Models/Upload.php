...
class Upload
{
	...
	public function handleVideo($request, $artistIds = null, $album_id = null, $isAdminPanel = false)
    {
        /** validate file format */
        if(config('settings.ffmpeg') && @shell_exec(env('FFMPEG_PATH') . ' -version')) {
            if(config('settings.max_video_file_size') == 0) {
                $request->validate([
                    'file.*' => 'required|mimetypes:application/octet-stream,audio/ogg,audio/x-wav,audio/wav,audio/mpeg,audio/flac,audio/x-hx-aac-adts,audio/x-m4a,video/mp4,video/x-ms-wma,audio/ac3,audio/aac',
                ]);
            } else {
                $request->validate([
                    'file.*' => 'required|mimetypes:application/octet-stream,audio/ogg,audio/x-wav,audio/wav,audio/mpeg,audio/flac,audio/x-hx-aac-adts,audio/x-m4a,video/mp4,video/x-ms-wma,audio/ac3,audio/aac|max:' . config('settings.max_video_file_size', 51200),
                ]);
            }
        } else {
            if(config('settings.max_video_file_size') == 0) {
                $request->validate([
                    'file.*' => 'required|mimetypes:video/mp4,application/octet-stream',
                ]);
            } else {
                $request->validate([
                    'file.*' => 'required|mimetypes:video/mp4,application/octet-stream|max:' . config('settings.max_video_file_size', 10240),
                ]);
            }
        }
        ...
        if ($request->file('file')->isValid()) {
            ...
            $getID3 = new \getID3;
            $mp4Info = $getID3->analyze($request->file('file')->getPathName());
			
            if(! isset($mp4Info['video']) || ! isset($mp4Info['video']['bitrate'])) {
                ...
                if(@shell_exec(env('FFMPEG_PATH') . ' -version')) {
                    ...
                    if(!isset($data['bitRate'])) {
                        ...
                        echo json_encode([
                            'message' => 'Not support',
                            'errors' => array('message' => array(__('Video file is not supported')))
                        ]);
                        exit;
                    }
                } else {
                ...
                $data = [
                    'bitRate' => intval($mp4Info['video']['bitrate'] / 1000),
                    'playtimeSeconds' => intval($mp4Info['playtime_seconds'])
                ];
            }

            Validator::make($data, [
                'bitRate' => ['required', 'numeric', 'min:' . config('settings.min_video_bitrate', 64), 'max:' . config('settings.max_video_bitrate', 320)],
                'playtimeSeconds' => ['required', 'numeric', 'min:' . config('settings.min_video_duration', 60), 'max:' . config('settings.max_video_duration', 300)],
            ])->validate();

            $trackInfo = array();

            if(isset($mp4Info['video']['id3v2']['title'][0])) {
                $trackInfo = $mp4Info['tags']['id3v2'];
            }elseif(isset($mp4Info['tags']['id3v2']['title'][0])) {
                $trackInfo = $mp4Info['tags']['id3v2'];
            } elseif(isset($mp4Info['tags']['id3v1'])) {
                $trackInfo = $mp4Info['tags']['id3v1'];
            } elseif(isset($mp4Info['id3v2']['comments']['title'][0])) {
                $trackInfo = $mp4Info['id3v2']['comments'];
            }
			...
			// exit;
            if (isset($mp4Info['comments']['picture']['0']['data']) || isset($trackInfo['picture']['0']['data'])) {
                $pictureData = isset($mp4Info['comments']['picture']['0']['data']) ? $mp4Info['comments']['picture']['0']['data'] : $trackInfo['picture']['0']['data'];
                $song->addMediaFromBase64(base64_encode(Image::make($pictureData)->fit(intval(config('settings.image_artwork_max', 500)),  intval(config('settings.image_artwork_max', 500)))->encode('jpg', config('settings.image_jpeg_quality', 90))->encoded))
                    ->usingFileName(Str::random(10) . '.jpg')
                    ->toMediaCollection('artwork', config('settings.storage_artwork_location', 'public'));
            }
