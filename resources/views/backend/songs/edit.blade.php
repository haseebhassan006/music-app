...
<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active"> {!! $song->title !!} - @foreach($song->artists as $artist)
        <a href="{{ route('backend.artists.edit', ['id' => $artist->id]) }}" title="{!! $artist->name !!}">{!! $artist->name !!}</a>
...
<div class="row">
    <div class="col-lg-12 media-info mb-3 album">
        <div class="media mb-3">
            ...
            <div class="media-body">
                <h5 class="m-0">{!! $song->title !!} - @foreach($song->artists as $artist)<a
                        href="{{ url('admin/artists/edit/' . $artist->id) }}" title="{!! $artist->name !!}">{!!
                        $artist->name !!}</a>@if(!$loop->last), @endif @endforeach</h5>
                <h5>
                    @if($song->mp3)
                ...
                <p class="m-0"><a href="{{ $song->permalink_url }}" class="btn btn-warning" target="_blank">Preview
                        @if(! $song->approved) (only Moderator) @endif</a></p>
            </div>
        </div>
    </div>
    <div class="col-lg-12 media-info mb-3 song">
        <iframe width="100%" height="60" frameborder="0"
            src="{{ asset('share/embed/dark/song/' . $song->id) }}"></iframe>
    </div>
        ...
        <form role="form" action="" enctype="multipart/form-data" method="post">
            ...
            <div class="card">
                <div class="card-header p-0 position-relative">
                    <ul class="nav">
                        <li class="nav-item"><a class="nav-link active" href="#overview" data-toggle="pill"><i
                                    class="fas fa-fw fa-newspaper"></i> Overview</a></li>
                        <li class="nav-item"><a href="#streamable" class="nav-link" data-toggle="pill"><i
                                    class="fas fa-fw fa-lock"></i> Advanced</a></li>
                        <li class="nav-item"><a href="#textspeech" class="nav-link" data-toggle="pill">Text to
                                speech</a></li>
                    </ul>
                ...
                <div class="card-body">
                    <div class="tab-content mt-2" id="myTabContent">
                        <div id="overview" class="tab-pane fade show active">
                            ...
                            <div class="form-group multi-artists">
                                ...
                                <select class="form-control multi-selector"
                                    data-ajax--url="{{ route('api.search.artist') }}" name="artistIds[]" multiple="">
                                    @foreach ($song->artists as $index => $artist)
                                    <option value="{{ $artist->id }}" selected="selected"
                                        data-artwork="{{ $artist->artwork_url }}" data-title="{!! $artist->name !!}">{!!
                                        $artist->name !!}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group select2-artwork">
                                <label>Album(s)</label>
                                <select class="form-control select-ajax" data-ajax--url="/api/search/album"
                                    name="albumIds[]">
                                    @if($song->album)
                                    <option value="{{ $song->album->id }}" selected="selected"
                                        data-artwork="{{ $song->album->artwork_url }}"
                                        data-title="{{ $song->album->title }}">{{ $song->album->title }}</option>
                                    @endif
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="input-group col-xs-12">
                                    ...
                                    <span class="input-group-btn">
                                        <button class="browse btn btn-primary input-lg" type="button"><i
                                                class="fas fa-fw fa-file"></i> Browse</button>
                                    </span>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <textarea rows="4" class="form-control"
                                    name="description">{{ $song->description }}</textarea>
                            </div>
                            <div class="form-group">
                                <label>Genres</label>
                            ...
                            <div class="form-group">
                                <label>Tags</label>
                                {!! makeTagSelector('tags[]', isset($song) && ! old('tags') ?
                                array_column($song->tags->toArray(), 'tag') : old('tags')) !!}
                            </div>
                            <div class="form-group">
                                <label>Released At</label>
                                <input type="text" class="form-control datepicker" name="released_at"
                                    value="{{ \Carbon\Carbon::parse($song->released_at)->format('m/d/Y') }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="youtube_id"
                                    value="{{ isset($song->log->youtube) ? $song->log->youtube : '' }}">
                            </div>
                            <div class="form-group">
                                <div class="col-sm-12">
                                    <label class="switch">
                        ...
                        <div id="streamable" class="tab-pane fade">
                            <div class="alert alert-info">Note: You can configure additional song playable and
                                downloadable parameters for different groups in this section.</div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    ], 'group_extra[' . $group->id . ']', isset($options) && isset($options[$group->id])
                                    ? $options[$group->id] : 0)
                                    !!}
                                </div>
                            </div>
                                ...
                                <form action="" method="post" id="ttsForm">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-4">
                                            <div class="card-header p-0 position-relative">
                                                <ul class="nav">
                                                    <li class="nav-item"><a class="nav-link active" href="#lovo"
                                                            data-toggle="pill">Lovo</a></li>
                                                    <li class="nav-item"><a href="#replica" class="nav-link"
                                                            data-toggle="pill">Replica</a></li>
                                                </ul>
                                            </div>
                                            <?php
                                            ...
                                            <div class="">
                                                <div class="tab-content mt-2" id="myTabContent">
                                                    <div id="lovo" class="tab-pane fade show active">
                                                        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion @if(isset($_COOKIE['sidebar']) &&  $_COOKIE['sidebar'] == 'small') toggled @endif "
                                                            id="accordionSidebarVoiceSkins"
                                                            style="width: 25rem !important;min-height: 0vh; border: solid 1px rgb(118, 118, 118);">
                                                            ...
                                                            <a class="collapse-item" href="javascript:void(0);" style="padding: 0px;">
                                                                ...
                                                                <span style="margin-left: 10px;">
                                                                    ...
                                                                    <span style="margin-left: 20px;">
                                                                        <audio controls="" style="outline: none; height: 25px; width: 90%;">
                                                                            <source src="<?php echo str_replace('https','http',$voiceList['links']['audio']);?>" type="audio/wav">
                                ...
                                <?php } ?>
                                                    ...
                                                    <div id="replica" class="tab-pane fade">
                                                        ...
                                                        foreach($replicaVoices as $replicaVoiceList){ ?>
                                                            <li class="nav-item zil_item" style="margin-bottom: 0rem;">
                                                                <div id="<?php echo $replicaVoiceList['name'];?>"
                                                                    class="" aria-labelledby="headingZila"
                                                                    data-parent="#accordionSidebarVoiceSkins"
                                                                    style="padding: 5px;"
                                                                    onclick='getSeakerId("<?php echo $replicaVoiceList['uuid'];?>","replica");'>
                                                                    <div class=" bg-dark  py-2 collapse-inner rounded">
                                                                        <a class="collapse-item"
                                                                            href="javascript:void(0);"
                                                                            style="padding: 0px; margin: 0 .5rem;">
                                                                            <i class="fas fa-user"></i>
                                                                            <span style="margin-left: 10px;">
                                                                                <label
                                                                                    id="speakerName<?php echo $replicaVoiceList['uuid'];?>"><?php echo $replicaVoiceList['name'];?></label>
                                                                                <br><br>
                                                                                <?php
																							if(isset($replicaVoiceList['preview_url']) && $replicaVoiceList['preview_url'] != ''){?>
                                                                                <span style="margin-left: 20px;">
                                                                                    <audio controls=""
                                                                                        style="outline: none; height: 25px; width: 90%;">
                                                                                        <source
                                                                                            src="<?php echo $replicaVoiceList['preview_url'];?>"
                                                                                            type="audio/wav">Your
                    ...
                    @if(! $song->approved)
                        <button type="button" class="btn btn-danger" data-toggle="collapse" data-target="#collapseExample"
                            aria-expanded="false" aria-controls="collapseExample">Reject</button>
                    @endif
                    <button type="reset" class="btn btn-secondary">Reset</button>
                    <a class="btn btn-danger" href="{{ route('backend.songs.delete', ['id' => $song->id]) }}" onclick="return confirm('Are you sure want to delete this song?')" data-toggle="tooltip" data-placement="left" title="Delete this song">
    ...
    <div class="col-lg-12">
        <div class="mt-5 collapse" id="collapseExample">
            <form role="form" method="post" action="{{ route('backend.songs.edit.reject.post', ['id' => $song->id]) }}"
...
function addDialogue() {
    ...
    totalDialouge++;
    var newDialogue = '<br><br><textarea class="sppeech_text" name="textspeech" id="text_speech' + totalDialouge +
        '"></textarea><div class="speech_button"><input type="hidden" id="textToSpeechType' + totalDialouge +
        '" value="" /><input type="hidden" id="textToSpeechSpeaker' + totalDialouge +
        '" value="" /><button type="button" class="btn microphone_btn" id="getValBtnID' + totalDialouge +
        '" onclick="getTextToSpeech(' + totalDialouge + ');">Play</button><span id="speaker' + totalDialouge +
        '" style="float: right; margin-right: 20px; margin-top: 5px; display:none;">Speaker: </span><audio id="spechid' +
        totalDialouge + '"  autoplay></audio></div>';
