...
function getSeakerId(speakerId,speakerType){
	if(speakerType == 'replica'){
		...
		$('#speaker').html('Speaker: '+$('#speakerName'+speakerId).text());
		$('#speaker').css('display','block');
		$('#textToSpeechType').val('replica');
	}
	else{
		speaker_id = speakerId;
		$('#speaker').html('Speaker: '+speakerId);
		$('#speaker').css('display','block');
		$('#textToSpeechType').val('lovo');
	}
	...
	$("#getValBtnID").on('click',(function(evt){
		if($('#textToSpeechType').val() == 'replica'){
			var formBody = [];
			const user={
			...
			if(speech == ''){
				...
				convert = 0;
				alert('Please select atleast any one voice fron Replica tab');
			}
		...
		else if($('#textToSpeechType').val() == 'lovo'){
			//alert();
			/*$.ajax({
			...
			$curl = curl_init();
				curl_setopt_array($curl, array(
				  ...
				  CURLOPT_POSTFIELDS => 'text=hello world! my name is Martha Sage&speaker_id=Susan Cole&output=sample.mp3&emphasis=1&pause=0&encoding=mp3',
				...
				/*if($response != ''){
					$mp3Path = asset("/backend/tts/lovoaudio8.mp3");
					
					if(file_exists(public_path()."/backend/tts/lovoaudio8.mp3"){
						echo 1;
					}
					else{
						echo 0;
